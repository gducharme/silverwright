...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { filename: @target.filename, label: @target.label } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { filename: @target.filename, label: @target.label } }
